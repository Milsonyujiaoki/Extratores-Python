...
MAX_SIMULTANEOUS = 4      # Máximo de requisições simultâneas (RAM, CPU, uso da API)
MAX_PER_SECOND = 3        # Máximo de requisições por segundo (rate OpenAI)

# Crie os limitadores globais
semaphore = asyncio.Semaphore(MAX_SIMULTANEOUS)
limiter = AsyncLimiter(MAX_PER_SECOND, time_period=1.0)  # 85 por segundo
    ...
    try:
        with open(path_csv_entrada_arquivos, "r", newline='', encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                # Verifica se já existe TXT ou JSON no subdiretório
                if os.path.exists(txt_output) or os.path.exists(json_output):
                    logger.info(f"[MAIN.GERAR_TXT_JSON_OPENAI] Pulando {nome}: TXT ou JSON já existe em {caminho_subdir}")
                    continue
                try:
                        ...
                        if results:
                            response = client.chat.completions.create(
                                ...
                                messages=[
                                    ...
                                    {"role": "user", "content": "Consolide os dados estruturados extraídos em um formato bem estruturado e organizado pelas paginas, ai em cada pagina tera suas datas(vencimento, pagamento,socilitacao, etc), valores monetarios."},
                                    {"role": "assistant", "content": "Claro! Vou consolidar os dados estruturados em um formato bem estruturado e organizado organizado."},
                    ...
                    logger.error(f"[MAIN.GERAR_TXT_JSON_OPENAI] Erro ao processar o arquivo {nome}: {e}")
                    continue
...
# Função síncrona para conversão de PDF para imagens
def pdf_to_images(pdf_path: Path) -> list[Path]:
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_dir_path = Path(temp_dir)
        pages = convert_from_path(str(pdf_path), dpi=300, output_folder=str(temp_dir))
        img_paths = []
        for i, page in enumerate(pages, 1):
            img_path = temp_dir_path / f"{pdf_path.stem}_page_{i}.png"
            page.save(img_path, "PNG")
            page.close()
            img_paths.append(img_path)
        # Copia os arquivos para um local fixo, se precisar acessar depois do bloco
        # ou retorna as imagens antes do bloco acabar
        # Exemplo: pode retornar uma lista de bytes ou salvar em outro lugar
        return img_paths  # Note: esses caminhos só existem dentro do bloco!

# Função assíncrona para processar uma imagem na OpenAI
async def process_image(image_path: Path, client: AsyncOpenAI) -> str:
    page_id = image_path.name
    logger.debug(f"[PAGE][{page_id}] Iniciando envio da página para OpenAI.")
    start_page = time.time()
    async with semaphore:
        async with limiter:
            try:
                logger.debug(f"[PAGE][{page_id}] Lendo e codificando imagem.")
                async with aiofiles.open(image_path, "rb") as img_file:
                    b64_image = base64.b64encode(await img_file.read()).decode("utf-8")
                logger.debug(f"[PAGE][{page_id}] Enviando para OpenAI.")
                logger.debug(f"[PAGE][{page_id}] Resposta recebida da OpenAI em {time.time()-start_page:.2f}s")
            ...
            temperature=0.2)
                ...
                logger.error(f"[PAGE][{page_id}] ERRO ao processar: {e}")
                raise

async def process_pdf_async(row, client: AsyncOpenAI, semaphore: asyncio.Semaphore):
    pdf_path = Path(row.get("caminho_arquivo"))
    nome_ = row.get("nome")
    nome = os.path.splitext(nome_)[0] if nome_ else ""
    caminho_arquivo = row.get("caminho_arquivo")
    caminho_subdir = row.get("caminho_subdir")
    # Verifica múltiplos padrões de nome para TXT/JSON
    padroes_txt = [f"{nome}.txt", f"openai_vision_{nome}.txt", f"openai_json_{nome}.txt"]
    padroes_json = [f"{nome}.json", f"openai_vision_{nome}.json", f"openai_json_{nome}.json"]
    arquivos_existentes = set(os.listdir(caminho_subdir)) if os.path.isdir(caminho_subdir) else set()
    existe_txt = any(p for p in padroes_txt if p in arquivos_existentes)
    existe_json = any(p for p in padroes_json if p in arquivos_existentes)
    logger.info(f"[PDF][{nome}] Iniciando processamento do PDF: {pdf_path}")
    if existe_txt or existe_json:
        logger.info(f"[PDF][{nome}] Pulando: TXT ou JSON já existe na pasta ({caminho_subdir}) com padrão alternativo.")
        return
    txt_output = Path(caminho_subdir) / f"{nome}.txt"
    json_output = Path(caminho_subdir) / f"{nome}.json"
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.debug(f"[PDF][{nome}] Convertendo PDF em imagens (pdf2image)...")
            image_paths = await loop.run_in_executor(None, pdf_to_images, Path(caminho_arquivo))
            logger.info(f"[PDF][{nome}] {len(image_paths)} páginas extraídas do PDF.")
            tasks = [process_image(img_path, client) for img_path in image_paths]
            logger.info(f"[PDF][{nome}] Enviando {len(tasks)} páginas para OpenAI Vision...")
            pages_results = await asyncio.gather(*tasks, return_exceptions=True)
            for i, result in enumerate(pages_results, 1):
                page_name = image_paths[i-1].name if image_paths else str(i)
                if isinstance(result, Exception):
                    logger.error(f"[PDF][{nome}][{page_name}] ERRO: {result}")
                    continue
                logger.info(f"[PDF][{nome}][{page_name}] Página processada.")
                results.append(f"=== DADOS ESTRUTURADOS - PÁGINA {i} ===\n")
            ...
            # Consolidação final
            if results:
                logger.info(f"[PDF][{nome}] Enviando dados para consolidação final (JSON)...")
                response = await client.chat.completions.create(
                    model="gpt-4.1-nano-2025-04-14",
                    messages=[
                        {"role": "system", "content": "Você é um assistente especializado na consolidação de dados estruturados."},
                ...
                consolidated_data = response.choices[0].message.content
                results_consolidador_json.append(consolidated_data)
                logger.info(f"[PDF][{nome}] Consolidação final recebida.")
            else:
                logger.warning(f"[PDF][{nome}] Nenhum dado estruturado encontrado.")

            # Salvando arquivos
            async with aiofiles.open(txt_output, "w", encoding="utf-8") as f:
                await f.write("".join(results))
            logger.info(f"[PDF][{nome}] TXT salvo em {txt_output}")
            async with aiofiles.open(json_output, "w", encoding="utf-8") as f:
                await f.write("".join(results_consolidador_json))
            logger.info(f"[PDF][{nome}] JSON salvo em {json_output}")

        logger.info(f"[PDF][{nome}] Processamento finalizado em {time.time() - start_pdf:.2f}s")
    except Exception as e:
        logger.error(f"[PDF][{nome}] ERRO inesperado: {e}")

async def gerar_txt_json_openAI_async(path_csv_entrada_arquivos: Path, client, max_api_calls: int = 10):
    import csv

    
    tasks = []
    async with aiofiles.open(path_csv_entrada_arquivos, "r", encoding="utf-8") as csvfile:
        content = await csvfile.read()
        reader = list(csv.DictReader(content.splitlines()))
    total_pdfs = len(reader)
    progresso = {"salvos": 0}

    logger.info(f"[BATCH] Iniciando processamento batch de {total_pdfs} PDFs.")

    async def wrapped_process_pdf_async(row, client, progresso):
        nome_ = row.get("nome")
        nome = os.path.splitext(nome_)[0] if nome_ else ""
        try:
            await process_pdf_async(row, client, semaphore)
            progresso["salvos"] += 1
            if progresso["salvos"] % 5 == 0 or progresso["salvos"] == total_pdfs:
                logger.info(f"[BATCH] Progresso: {progresso['salvos']}/{total_pdfs} PDFs processados.")
        except Exception as e:
            logger.error(f"[BATCH][{nome}] ERRO ao processar PDF: {e}")

    for row in reader:
        tasks.append(wrapped_process_pdf_async(row, client, progresso))

    start_batch = time.time()
    await asyncio.gather(*tasks)
    logger.info(f"[BATCH] Processamento de todos os PDFs finalizado em {time.time()-start_batch:.2f}s.")

    
def main():
    global logger, config, api_key
    ...
    asyncio.run(gerar_txt_json_openAI_async(path_csv_entrada_arquivos, client, max_api_calls=4))
