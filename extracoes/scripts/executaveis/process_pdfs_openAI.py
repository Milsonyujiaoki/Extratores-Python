...
MAX_SIMULTANEOUS = 6      # Máximo de requisições simultâneas (RAM, CPU, uso da API)
MAX_PER_SECOND = 6        # Máximo de requisições por segundo (rate OpenAI)
...
async def process_pdf_async(row, client: AsyncOpenAI, semaphore: asyncio.Semaphore):
    ...
    if txt_output.exists() or json_output.exists():
        logger.info(f"[PDF][{nome}] Pulando: TXT ou JSON já existe.")
        return
...
async def gerar_txt_json_openAI_async(path_csv_entrada_arquivos: Path, api_key="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", max_api_calls: int = 10):
    import csv
    print("AsyncOpenAI:", AsyncOpenAI)
    print("AsyncOpenAI:", AsyncOpenAI)
    print("AsyncOpenAI type:", type(AsyncOpenAI))
    print("api_key:", api_key, type(api_key))
    print("API KEY:", repr(api_key), type(api_key))
...
def main():
    ...
    asyncio.run(gerar_txt_json_openAI_async(path_csv_entrada_arquivos, api_key, max_api_calls=4))
